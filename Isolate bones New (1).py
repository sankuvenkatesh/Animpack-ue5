bl_info = {
    ...
    "description": "Hides/shows bones based on selection. With bone controllers overlay toggle.",
    "category": "Animation",
}
...
class VIEW3D_PT_BoneIsolationPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Bone Controllers overlay toggle in box
        show_bones = getattr(getattr(space, "overlay", None), "show_bones", None)
        if show_bones is not None:
            box.prop(space.overlay, "show_bones", text="Show Bone Controllers")
        else:
            box.label(text="No Bone overlay property.", icon='ERROR')

        col = box.column(align=True)
        col.operator("armature.isolate_bones", text="Isolate Selected", icon='RESTRICT_SELECT_OFF')

        row = col.row(align=True)
        row.operator("armature.show_all_bones", text="Show All Bones", icon='HIDE_OFF')
